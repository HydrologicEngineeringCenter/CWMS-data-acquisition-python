...
def parse_nws_loc_parm_data(locId, param):
    ...
    if ('ROR' in locId[5::])  & ("Flow" in param):
        param = param +'-RainOnPool' 
...
Locdf = cwms.get_location_group(loc_group_id  = "NWS Handbook 5 ID",category_id= "Agency Aliases", 
                                category_office_id= "CWMS", group_office_id="CWMS", office_id=OFFICE).df.set_index('location-id')
...
tmp_df = cwms.get_location_group(loc_group_id  = "Locations",category_id= "RFC CHPS Aliases", 
                                 category_office_id=OFFICE, group_office_id=OFFICE, office_id = OFFICE).df.set_index('location-id')
#Grab all of the locations that have a nws ID assigned to them
...
tmp_df['Loc_attribute']=0

# drop the rows to be replaced
nws_alias = nws_alias.drop(nws_alias.index.intersection(tmp_df.index))
# replace the rows
nws_alias = pd.concat([nws_alias, tmp_df])
...
if 'auto' in filename:
    ...
    versionDateTime = fDateTime.replace(hour=1, minute=11, second=0,  tzinfo=timezone.utc)
